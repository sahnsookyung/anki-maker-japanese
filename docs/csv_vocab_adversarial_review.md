...
This review covers the CSV-only vocabulary-note change after shifting vocab pages from three duplicate review cards to one `jp_vocab_entry` note candidate per `(Surface, Reading)` pair.
...
- A vocab note with every study direction disabled generates no useful Anki card. The UI should present study directions as optional templates on one note, not as duplicate review cards, and the export path should omit explicit zero-output notes.
...
- One `jp_vocab_entry` note per surface/reading pair is the right Anki model. It removes duplicate review-state noise in the UI while letting one Anki template generate the pronunciation-to-kanji card and hide support fields such as Korean meaning.
...
The strongest argument is that the note-model change is correct, but the implementation should be stricter about the visible/generated card shape. The code should delete unsupported vocab card shapes, reject ambiguous single-file CSV exports, represent study directions as conditional templates on one note, prevent zero-card vocab notes, and make exported CSV files available through stable UI links. The benchmark should continue reporting low vocab accuracy honestly; improving OCR extraction is a separate pipeline task, not something the export layer should hide.
...
- Export writes `jp_vocab_entry` notes with required facts and study-direction flags; the default is one generated kana-to-kanji card.
- The vocab editor shows one note preview with enabled Anki templates while keeping one semantic note per vocab row.
